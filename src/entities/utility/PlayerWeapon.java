...
public class PlayerWeapon {
	...
	public boolean tryToFire() {
		...
		lastFireTime = System.currentTimeMillis();
		int x = ship.getX() + ship.getAnimation().getDimensionX() / 2
				- ImageManager.getImage(bulletName).getWidth(null) / 2;
		int y = ship.getY() - ImageManager.getImage(bulletName).getHeight(null);
	...
	public void createShot(final int x, final int y) {
		...
		case "projectiles/shot5":
			for (int i = -800; i <= 800; i += 100) {
				game.getEntityManager().addToEntities(new ShotEntity(game, bulletName, x + i, y, 0));
			}
			break;
		case "projectiles/shot6":
			for (int i = 0; i <= 180; i += 30) {
				game.getEntityManager().addToEntities(new ShotEntity(game, bulletName, x - 30 + i / 3,
